...
export default async function OrgMembersPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="p-6 bg-slate-50/50 min-h-screen space-y-6 max-w-7xl mx-auto">
      {/* 헤더 타이틀 */}
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          {organization.name}{" "}
          <span className="text-slate-400 text-xl font-medium">회원 관리</span>
        </h2>
        <p className="text-slate-500 mt-2 text-sm">
          이 단체에 등록된 회원 목록을 확인하고 권한 및 상태를 관리하세요.
        </p>
      </div>
